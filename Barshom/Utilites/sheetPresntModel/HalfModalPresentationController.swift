  ...
  override init(presentedViewController: UIViewController, presenting presentingViewController: UIViewController?) {
    ...
    @objc func onPan(pan: UIPanGestureRecognizer) -> Void {
        ...
        case .changed:
            ...
            switch state {
            case .normal:
              presentedView!.frame.origin.y = endPoint.y + containerView!.frame.height / screenHeight
            case .adjustedOnce:
    ...
    func changeScale(to state: ModalScaleState) {
        if let presentedView = presentedView, let containerView = self.containerView {
            UIView.animate(withDuration: 0.8, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: .curveEaseInOut, animations: { () -> Void in
              ...
              let halfFrame = CGRect(origin: CGPoint(x: 0, y: containerFrame.height / self.screenHeight),
                                     size: CGSize(width: containerFrame.width, height: containerFrame.height - (containerFrame.height / self.screenHeight)))
    ...
    override var frameOfPresentedViewInContainerView: CGRect {
      return CGRect(x: 0, y: containerView!.bounds.height / screenHeight, width: containerView!.bounds.width, height: containerView!.bounds.height-(containerView!.bounds.height / screenHeight))
    }
